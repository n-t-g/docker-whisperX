...
def process_files(args, pipelines):
    """
    Process all audio files in args using the loaded pipelines and write output files.
    """
    ...
    results = []
    # Process each file with the ASR pipeline.
    ...
    if not config["no_align"]:
        ...
        for result, audio_path in tmp_results:
            # If more than one file, use the file as input; otherwise, reuse the previously loaded audio.
            input_audio = audio_path if len(tmp_results) > 1 else load_audio(audio_path)
            if pipelines["align"] is not None and len(result["segments"]) > 0:
                # If the detected language differs from the alignment model language, reload.
                if result.get("language", "en") != pipelines["align_metadata"]["language"]:
                    ...
                    pipelines["align"], pipelines["align_metadata"] = load_align_model(result["language"], config["device"])
    ...
    if config["diarize"] and pipelines["diarize"] is not None:
        ...
        for result, audio_path in tmp_results:
            diarize_segments = pipelines["diarize"](audio_path, min_speakers=config["min_speakers"], max_speakers=config["max_speakers"])
